...
export const Approval: React.FC = () => {
    ...
    const handleUpdateSuccess = async () => {
        // Refetch data and update the selectedRequest in-place so the modal shows the new status
        try {
            const [reqs, temps] = await Promise.all([
                getRequests(),
                getTemplates()
            ...
            if (selectedRequest) {
                const updatedReq = reqs.find(r => r.id === selectedRequest.id);
                if (updatedReq) {
        ...
        // Do NOT close the modal – let the user see the updated status
    };
