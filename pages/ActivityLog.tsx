...
export const ActivityLog: React.FC = () => {
    ...
    return (
        <div className="px-2 sm:px-4 md:px-6 py-4 sm:py-5 md:py-6 space-y-3 sm:space-y-4 md:space-y-6 w-full animate-in fade-in duration-500">
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 sm:gap-3 md:gap-4">
                <div className="flex items-center gap-2 sm:gap-3 md:gap-4 min-w-0">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-accent text-white rounded-lg sm:rounded-2xl flex items-center justify-center shadow-hard border-2 border-slate-900 flex-shrink-0">
                        <History size={18} className="sm:w-6 sm:h-6" />
                    </div>
                    <div className="min-w-0">
                        <h1 className="text-lg sm:text-xl md:text-2xl font-black text-slate-800 truncate">Log Aktivitas Tim</h1>
                        <p className="text-[10px] sm:text-xs md:text-sm font-bold text-slate-500 italic">Pantau setiap perubahan dan aksi dalam tim Anda.</p>
                    </div>
                </div>
                <Button
                    variant="secondary"
                    onClick={loadLogs}
                    isLoading={loading}
                    icon={<RefreshCcw size={14} className="sm:w-4 sm:h-4 md:w-[18px] md:h-[18px]" />}
            ...
            {/* Filters */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-4 bg-white p-3 sm:p-4 rounded-lg sm:rounded-3xl border-3 sm:border-4 border-slate-900 shadow-hard-mini">
                <div className="relative col-span-1 sm:col-span-2 lg:col-span-1">
                    ...
                    <input
                        ...
                        className="w-full pl-8 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-3 bg-slate-50 border-2 border-slate-200 rounded-lg sm:rounded-xl focus:border-slate-900 focus:ring-0 transition-all font-bold text-xs sm:text-sm"
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                    />
                </div>
                <div className="relative">
                    <Layers className="absolute left-2.5 sm:left-3 top-1/2 -translate-y-1/2 text-slate-400" size={14} className="sm:w-4 sm:h-4" />
                    <select
                        className="w-full pl-8 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-3 bg-slate-50 border-2 border-slate-200 rounded-lg sm:rounded-xl focus:border-slate-900 focus:ring-0 transition-all font-bold text-xs sm:text-sm appearance-none"
