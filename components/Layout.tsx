...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        if (icon192 || icon512 || iconMask) {
            ...
            if (config?.app_icon_192?.startsWith('https://')) {
                if (!appleLink) {
        ...
        <>
            <div className={`flex h-screen w-full overflow-hidden bg-background relative theme-${currentTheme}`}>
                ...
                <div className={`flex flex-col h-screen overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] w-full min-w-0 ${isSidebarOpen ? 'md:pl-72' : 'pl-0 md:pl-20'}`}>
                    ...
                    <main className={`flex-1 flex flex-col overflow-y-auto overflow-x-hidden custom-scrollbar min-h-0 bg-background w-full pb-20 sm:pb-20 md:pb-8 lg:pb-10 ${location.pathname.startsWith('/carousel') ? 'p-2 sm:p-3 md:p-4 md:px-6 md:py-8' : 'p-4 sm:p-4 md:p-6 md:py-6 lg:px-6 lg:py-8'}`}>
